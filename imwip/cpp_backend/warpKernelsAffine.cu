...
__global__ void affineCubicWarp3DKernelMul(
        const float* f,
        const float* A,
        const float* b,
        float* fWarped,
                ...
                for(int kk = 0; kk < 4; kk++){
                    ...
                    && 0 <= Q2 && Q2 < shape2){
                        ...
                        for(int l = 0; l < shape3; l++){
                            fWarped[i*shape1*shape2*shape3 + j*shape2*shape3 + k*shape3 + l] += coefficient * f[Q0*shape1*shape2*shape3 + Q1*shape2*shape3 + Q2*shape3 + l];
                        }
                    }
                    m++;
                }
