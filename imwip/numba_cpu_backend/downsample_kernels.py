...
def cubic_downsampling_3D_kernel(f, f_lr, coeffs):
    for i in prange(f_lr.shape[0]):
        for j in prange(f_lr.shape[1]):
            for k in prange(f_lr.shape[2]):
                f_i = float32(i)
                f_j = float32(j)
                f_k = float32(k)
...
@njit(fastmath=True)
def adjoint_cubic_downsampling_3D_kernel(f, f_lr, coeffs):
